...
def apply_spatial_audio(player, column_index, total_columns,
    ...
    if total_columns <= 1:
        active_player.audio_set_channel(vlc.AudioOutputChannel.Stereo)
    ...
    if column_index < midpoint:
        active_player.audio_set_channel(vlc.AudioOutputChannel.Left)
    elif column_index > midpoint:
        active_player.audio_set_channel(vlc.AudioOutputChannel.Right)
    else:
        active_player.audio_set_channel(vlc.AudioOutputChannel.Stereo)
